...
public class CarbonProvider extends Provider {
	...
	// LoggerFactory.getLogger(CarbonProvider.class);
		...
		thread = new Thread(new Runnable() {
			public void run() {
				synchronized (lock) {
					...
					int status = Lib.InstallEventHandler(Lib.GetEventDispatcherTarget(), keyListener, 1, eventTypes,
							null, eventHandlerReference); // fHandlerRef
					if (status != 0) {
	...
	private void register(OSXHotKey hotKey) {
		...
		int status = Lib.RegisterEventHotKey(KeyMap.getKeyCode(keyCode), KeyMap.getModifier(keyCode), hotKeyReference,
				Lib.GetEventDispatcherTarget(), 0, gMyHotKeyRef);

		if (status != 0) {
			// LOGGER.warn("Could not register HotKey: " + keyCode + ". Error code: " +
			// status);
			return;
		}
	...
	private class EventHandler implements Carbon.EventHandlerProcPtr {
		public int callback(Pointer inHandlerCallRef, Pointer inEvent, Pointer inUserData) {
			...
			int ret = Lib.GetEventParameter(inEvent, kEventParamDirectObject, typeEventHotKeyID, null,
